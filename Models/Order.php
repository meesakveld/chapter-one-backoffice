...
class Order extends BaseModel
{
    ...
    protected function getOrderWithId($orderId)
    {
        $sql = "
        SELECT 
            orders.id,
        ...
        $result = $stmt->fetchObject(Order::class);
    ...
    public function getOrderItems() {
        $sql = "
            ...
            FROM 
                book_order
            LEFT JOIN 
                books ON book_order.book_id = books.id
            WHERE 
                book_order.order_id = :orderId;
        ";
        $stmt = $this->db->prepare($sql);
        $stmt->execute(['orderId' => $this->id]);
        $result = $stmt->fetchAll(PDO::FETCH_CLASS, BookOrder::class);
